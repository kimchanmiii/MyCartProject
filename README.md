...
상품이 3개가 화면에 출력되고 각 상품의 이미지와 제품명, 가격 정보가 표시된다. 상품 선택하면 아래의 장바구니 혹은 구매 버튼을 눌러 해당 페이지로 넘어갈 수 있다.
...
홈 화면에서 선택한 상품이 장바구니 페이지에 넘어온다. 추가된 상품의 상품명, 가격 정보가 출력된다. 화면 아래의 홈 버튼과 구매버튼을 통해 해당 페이지로 넘어갈 수 있다. 상품별로 체크박스 선택여부 확인 후에 선택된 제품만 구매페이지로 넘어간다. 
...
장바구니에서 구매하려고 선택한 제품만 구매화면에 출력된다. 선택한 상품의 제품명과 가격 정보를 화면에 출력한다. 선택한 제품들의 결제할 총 금액이 화면에 출력된다. 정보를 확인 후 구매자는 주소정보와 연락처를 입력할 수 있다. 정보를 입력하지 않은 경우 정보를 입력하라는 팝업창이 출력되고, 주소와 연락처가 입력된 경우 구매를 완료할 것인지 묻는 팝업창이 출력된다. 확인을 누르면 첫번째 페이지(홈)으로 이동한다.
